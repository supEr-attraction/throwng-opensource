...
const RhythmCouponPage = () => {
  const [coupon, setCoupon] = useState<quizCoupon | null>(null);
  const [loading, setLoading] = useState<boolean>(true);
  const navigate = useNavigate();

  useEffect(() => {
    const scoreAchieved = sessionStorage.getItem("scoreAchieved") || "false";
    ...
    const fetchCoupon = async () => {
      setLoading(true);
      try {
        const couponData = await getContentCoupon("game");
...
export default RhythmCouponPage;
